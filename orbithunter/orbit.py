from math import pi
from orbithunter._arrayops import swap_modes, so2_generator, so2_coefficients
from orbithunter.discretization import rediscretize
from orbithunter.generate import random_initial_condition
...
class OrbitKS:
    ...
    **kwargs :
    ...
    def status(self):
        ...
        if zero_check < self.N*self.M*10**-10:
            return self.__class__(state=np.zeros([self.N, self.M]),
                                  statetype='field', T=self.T, L=self.L, S=self.S), 4
    ...
    def dt(self, order=1):
        ...
        orbit_dtn = self.__class__(state=dtn_modes, statetype='modes', T=self.T, L=self.L, S=self.S)
        return orbit_dtn
    ...
    def dx(self, order=1):
        ...
        orbit_dxn = self.__class__(state=dxn_modes, statetype='modes', T=self.T, L=self.L, S=self.S)
        return orbit_dxn
    ...
    def jacobian(self, fixedparams=(False, False)):
    ...
    def matvec(self, other, fixedparams=(False, False), preconditioning=True):
        ...
        orbit_linear = self.__class__(state=linear_term, statetype='modes', T=self.T, L=self.L)
    ...
    def pseudospectral(self, other):
        ...
        # Return Spatial derivative with 1/2 factor
    ...
    def reflection(self):
        ...
        return self.__class__(state=reflected_field, statetype='field', T=self.T, L=self.L, S=-1.0*self.S)
...
class RelativeOrbitKS(OrbitKS):
    ...
    def change_reference_frame(self):
        ...
        frame_change_ = self.__class__(state=frame_rotated_s_modes, statetype='s_modes',
    ...
    def jacobian(self, fixedparams=(False, False, False)):
    ...
    def matvec(self, other, fixedparams=(False, False, False), preconditioning=True, **kwargs):
        ...
        orbit_linear = self.__class__(state=linear_term, statetype='modes', T=self.T, L=self.L, S=self.S)
...
class ShiftReflectionOrbitKS(OrbitKS):
    ...
    def dx(self, order=1):
        ...
        if np.mod(order, 2):
            ...
            return self.__class__(state=dxn_s_modes, statetype='s_modes', T=self.T, L=self.L).convert(to='s_modes')
        else:
