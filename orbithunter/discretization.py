...
def _parameter_based_discretization(parameters, **kwargs):
    ...
    if kwargs.get('N', None) is None:
        if resolution == 'coarse':
            N = np.max([2**(int(np.log2(T)-2)), 32])
        elif resolution == 'fine':
            N = np.max([2**(int(np.log2(T)+4)), 32])
        else:
            N = np.max([2**(int(np.log2(T))), 32])
    else:
        N = kwargs.get('M', None)

    if kwargs.get('M', None) is None:
        if resolution == 'coarse':
            M = np.max([2**(int(np.log2(L)-1)), 32])
        elif resolution == 'fine':
...
def rediscretize(orbit, parameter_based=False, **kwargs):
    ...
    if parameter_based:
        new_N, new_M = _parameter_based_discretization(orbit.parameters, **kwargs)
    else:
