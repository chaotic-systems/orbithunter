        ...
        if dirpath.count('fail') != 0:
            ...
            for f in filenames:
                if f.split('_')[0] in ['Orbit', 'AntisymmetricOrbit',
                    ...
                    continue
                cls, _ = parse_class(f)
                if cls is ShiftReflectionOrbit and f.endswith('.h5'):
                    if dirpath.count('block') != 0:
                        # Different naming format for tiles/blocks
                        x = read_h5(f, directory=dirpath)
                        x.convert(to='modes')
                    ...
                    if os.path.isfile(savename) and overwrite == False:
                        ...
                        print(f, [x.N, x.M])
                        sys.stdout.flush()
                        y = RelativeOrbit(state=x.to_fundamental_domain().state, statetype='field', T=x.T, L=x.L)
                        y.plot(fundamental_domain=False)
                        y.calculate_shift(inplace=True)
                        y.plot(fundamental_domain=False)
                        result = converge(x, verbose=True)
